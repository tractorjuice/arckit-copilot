...
```bash
.arckit/scripts/generate-document-id.mjs <PROJECT_NUMBER> FSCTP --filename
```
...
Before writing, read `.arckit/references/quality-checklist.md` and verify all **Common Checks** plus the **FSCTP** per-type checks pass. Fix any failures before proceeding.
