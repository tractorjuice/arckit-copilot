...
- **If not found**: Read `.arckit/templates/architecture-board-template.md` (default)
...
- **Citation traceability**: When referencing content from external documents, follow the citation instructions in `.arckit/references/citation-instructions.md`. Place inline citation markers (e.g., `[BD-C1]`) next to findings informed by source documents.
...
- `[YYYY-MM-DD]` for Next Review Date → Current date + 90 days (quarterly review)
