...
```bash
.arckit/scripts/generate-document-id.mjs <PROJECT_NUMBER> FSCD --filename
```
...
- `.arckit/templates/uk-fs-consumer-duty-template.md` — master artefact template
- `.arckit/templates/uk-fs-consumer-duty-board-report-template.md` — board attestation
...
Before writing, read `.arckit/references/quality-checklist.md` and verify all **Common Checks** plus the **FSCD** per-type checks pass. Fix any failures before proceeding.
