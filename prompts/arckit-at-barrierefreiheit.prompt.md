...
- **REQ** (Requirements) — Extract: NFR-UX / accessibility requirements, user-facing functional scope (FR-xxx), channels and interfaces
  - If missing: warn that conformance cannot be scoped without knowing which surfaces exist. Proceed only on the description in `${input:topic:Enter project name or topic}`.
...
Identify the target project from the hook context. If the project doesn't exist, create it with `.arckit/../arckit-claude/scripts/bash/create-project.sh --json --name "<project-name>"` — the `--name` is required, and without it the script exits 1 rather than returning a path.
...
Read `.arckit/templates-custom/at-barrierefreiheit-template.md` if it exists, otherwise `.arckit/templates/at-barrierefreiheit-template.md`.
...
2. **Auto-populate Document Control**: resolve the `<!-- DOC-CONTROL-HEADER -->` marker per `.arckit/templates/_partials/RENDERING.md`.
...
**Populate the External References section** per `.arckit/references/citation-instructions.md`. BaFG (BGBl. I Nr. 76/2023), WZG (BGBl. I Nr. 59/2019), Directive (EU) 2019/882, Directive (EU) 2016/2102, EN 301 549 v3.2.1 and WCAG 2.1 MUST appear in the Document Register.

Before writing the file, read `.arckit/references/quality-checklist.md` and verify all **Common Checks** plus the **ATBFR** per-type checks pass.
