...
1. If the user's `${input:topic:Enter project name or topic}` contains an explicit `projects/{NNN}-{name}/` path, use that path verbatim.
2. If `${input:topic:Enter project name or topic}` contains a bare project number (e.g. `002`) or name fragment, glob `projects/{NUMBER}-*/` or `projects/*-*{NAME}*/` and use the unique match. If multiple match, ask the user to disambiguate before proceeding — do not default to "most recent".
...
using the explicit scope in `${input:topic:Enter project name or topic}` and say so in your first message
(e.g. "No requirements found — scoping the competitor query from your
arguments only").

### Step 2: Parse scope → reader input

From `${input:topic:Enter project name or topic}`, after stripping the project hint:
